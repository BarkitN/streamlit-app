...
BASE_FILL = PatternFill(fill_type="solid", fgColor="D9EAD3")
DOUBLE_FILL = PatternFill(fill_type="solid", fgColor="D9EAF7")
TRIPLE_FILL = PatternFill(fill_type="solid", fgColor="F4CCCC")
RECO_FILL = PatternFill(fill_type="solid", fgColor="FFF2CC")
...
def build_playable_combinations(show_df: pd.DataFrame, max_rows: int = 50000):
    if show_df is None or show_df.empty:
        return pd.DataFrame(), pd.DataFrame()
    options_per_match = []
    summary_rows = []
    for _, row in show_df.iterrows():
        ticket = str(row["Ticket"]).strip().upper()
        if ticket == "":
            opts = []; typ = "VIDE"
        elif ticket in {"1", "X", "2"}:
            opts = [ticket]; typ = "BASE"
        elif ticket in {"1X", "12", "X2"}:
            opts = list(ticket); typ = "DOUBLE"
        else:
            opts = ["1", "X", "2"]; typ = "TRIPLE"
        options_per_match.append(opts)
        summary_rows.append({
            "N°": row["N°"], "Match": row["Match"], "League": row["League"], "Ticket": ticket,
            "Type": typ, "Choix_1": opts[0] if len(opts) > 0 else "", "Choix_2": opts[1] if len(opts) > 1 else "",
            "Choix_3": opts[2] if len(opts) > 2 else "", "Recommendation": row["Recommendation"],
        })
    total = 1
    for opts in options_per_match:
        total *= max(1, len(opts))
    summary_df = pd.DataFrame(summary_rows)
    summary_df["Total_Combinations_Possibles"] = total
    if total > max_rows or any(len(opts) == 0 for opts in options_per_match):
        return summary_df, pd.DataFrame()
    combos = []
    for idx, picks in enumerate(product(*options_per_match), start=1):
        rec = {"Combinaison_ID": idx}
        for i, pick in enumerate(picks, start=1):
            rec[f"M{i}"] = pick
        rec["Ticket_Compact"] = " | ".join(picks)
        combos.append(rec)
    return summary_df, pd.DataFrame(combos)

def style_export_sheet(ws, sheet_name: str):
    for cell in ws[1]:
        cell.fill = HEADER_FILL
        cell.font = HEADER_FONT
    if sheet_name in {"Final_Ticket", "Combinaisons_Resume"}:
        headers = {ws.cell(row=1, column=c).value: c for c in range(1, ws.max_column + 1)}
        type_like_col = headers.get("Level") or headers.get("Type")
        reco_col = headers.get("Recommendation")
        for r in range(2, ws.max_row + 1):
            if type_like_col:
                val = str(ws.cell(r, type_like_col).value or "")
                fill = BASE_FILL if val == "BASE" else DOUBLE_FILL if val == "DOUBLE" else TRIPLE_FILL
                for c in range(1, ws.max_column + 1):
                    ws.cell(r, c).fill = fill
            if reco_col:
                ws.cell(r, reco_col).fill = RECO_FILL
...
def to_excel_bytes(sheets):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for ws in writer.book.worksheets:
            style_export_sheet(ws, ws.title)
    output.seek(0)
...
st.markdown('<div class="hero"><h1>MAX PRO - V3 AI PREDICTOR FULL ELITE</h1><p class="small-note">Version renforcée : auto-détection CSV, ligues inférées automatiquement, consensus enrichi, ticket final, niveau de risque, combinaisons jouables et export Excel coloré.</p></div>', unsafe_allow_html=True)
...
combo_summary_df, combos_df = build_playable_combinations(show)

tabs = st.tabs(["WEB DB import", "Historiques ligues", "Historique profond", "Saisie sources", "Consensus MAX PRO", "Ticket final", "Combinaisons", "Debug V3", "Export Excel"])
...
with tabs[6]:
    st.subheader("Combinaisons jouables et possibles")
    if not combo_summary_df.empty:
        b = int((combo_summary_df["Type"] == "BASE").sum())
        d = int((combo_summary_df["Type"] == "DOUBLE").sum())
        t = int((combo_summary_df["Type"] == "TRIPLE").sum())
        total = int(combo_summary_df["Total_Combinations_Possibles"].iloc[0])
        s1, s2, s3, s4 = st.columns(4)
        s1.metric("Bases", b)
        s2.metric("Doubles", d)
        s3.metric("Triples", t)
        s4.metric("Combinaisons possibles", total)
        st.markdown("### Résumé des choix recommandés")
        st.dataframe(combo_summary_df, use_container_width=True, height=360, hide_index=True)
        st.markdown("### Ensemble des combinaisons jouables")
        if combos_df.empty:
            st.warning("Le nombre de combinaisons est trop élevé pour être affiché entièrement. Le résumé sera bien exporté dans Excel.")
        else:
            st.dataframe(combos_df, use_container_width=True, height=360, hide_index=True)
...
with tabs[8]:
    ...
    payload = {
        ...
        "Final_Ticket": show,
        "Combinaisons_Resume": combo_summary_df,
        "Combinaisons_Jouables": combos_df,
