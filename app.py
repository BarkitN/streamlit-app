...
with tabs[5]:
    ...
    if consensus_df.empty:
        show = pd.DataFrame(columns=["N°", "Match", "League", "Ticket", "Level", "Confidence", "Historical_Pick", "Historical_Double", "Recommendation"])
    else:
        show = consensus_df[["N°", "Match", "League", "Ticket", "Level", "Confidence", "Historical_Pick", "Historical_Double"]].copy()
        show["Recommendation"] = show.apply(
            lambda r: "Jouer en base" if r["Level"] == "BASE" else (f"Couvrir en {r['Ticket']}" if r["Level"] == "DOUBLE" else "Match très piégeux"),
            axis=1,
        )
...
with tabs[6]:
    ...
    st.download_button(
        "Télécharger l'Excel MAX PRO V2",
        data=to_excel_bytes(payload),
        file_name="MAX_PRO_NEW_GRID_PROJECT_V2.xlsx",
